...
def create_flashcard_player():
    """Generates a Universal HTML Player with Tap-to-Sync."""
    html_content = """<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Group Karaoke</title>
    <style>
        ...
        .line:active { background: rgba(255, 255, 255, 0.1); } /* Visual feedback on tap */
...
<body>
    ...
    <div id="toast">Synced to line!</div>
    ...
    <script>
        ...
        // Detect user scrolling interaction to pause auto-scroll
        container.addEventListener('touchstart', () => { isUserScrolling = true; });
        container.addEventListener('touchend', () => { 
            clearTimeout(scrollTimeout);
            scrollTimeout = setTimeout(() => { isUserScrolling = false; }, 2000); // Resume auto-scroll after 2s
        });
        ...
        function seekTo(index) {
            if (lyrics[index].time !== -1) {
                audioPlayer.currentTime = lyrics[index].time;
                audioPlayer.play();
                showToast("Synced!");
            }
            // For TXT mode, just highlight
        ...
        function parseTXT(text) {
            lyrics = [];
            text.split('\\n').forEach(line => { if(line.trim()) lyrics.push({ time: -1, text: line.trim() }); });
            renderLyrics();
        }
        ...
        audioPlayer.addEventListener('timeupdate', () => {
            ...
            for (let i = 0; i < lyrics.length; i++) {
                if (audioPlayer.currentTime >= lyrics[i].time) activeIndex = i; else break;
            }
            if (activeIndex !== -1) {
                document.querySelectorAll('.line').forEach(d => d.classList.remove('active'));
                const a = document.getElementById(`line-${activeIndex}`);
                if (a) { 
